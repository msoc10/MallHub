...
export default function StoreDiscountForm({ onDiscountChange }) {
  ...
  const fetchCurrentDiscount = async () => {
    try {
      const discountData = await AdminService.getStoreDiscount();
      setCurrentDiscount(discountData.discount_percentage);
  ...
  const handleApplyDiscount = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await AdminService.applyStoreDiscount(discountPercentage);
      setCurrentDiscount(result.discount_percentage);
      ...
      if (onDiscountChange) {
        onDiscountChange(result.discount_percentage);
      }
    } catch (error) {
  ...
  const handleRemoveDiscount = async () => {
    setIsRemoving(true);
    try {
      const result = await AdminService.removeStoreDiscount();
