...
class AdminService extends BaseService {
    ...
    async getProduct(id) {
        ...
        data.product.is_pre_order = Boolean(data.product.is_pre_order);
        return data.product;
    }
    ...
    async createProduct(input) {
        ...
        formData.append("is_pre_order", input.is_pre_order === true ? "true" : "false")
        const { data } = await this.post(`create-product/`, formData);
        return data;
    }

    async updateProduct(input) {
        console.log("Updating product with data:", input);
        const formData = new FormData();
        formData.append("name", input.name)
        formData.append("description", input.description)
        formData.append("price", input.price)
    ...
    // Store discount methods
    async getStoreDiscount() {
        const { data } = await this.get(`my-store/discount/`);
        return data;
    }
    
    async applyStoreDiscount(discountPercentage) {
        const { data } = await this.post(`my-store/discount/`, {
