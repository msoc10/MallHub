...
const StoreProductsPage = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        setLoading(true);
        const data = await StoreService.getStoreProducts({ id, ...pagination });
        setData(prev => ({ ...prev, items: [...prev.items, ...data.items] }))
      } catch (ex) {
        console.log(ex)
      } finally {
        setLoading(false);
    ...
    <div className="flex-1">
      ...
      <section id="products" className="w-full wrapper">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {data?.items?.map((src, index) => (<ProductCard key={index} {...src} />))}
        </div>
