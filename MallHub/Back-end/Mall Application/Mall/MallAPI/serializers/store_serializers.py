...
from MallAPI.models.store_model import Store, Product, Category,Section, ProductComment, ProductInteraction, CommentInteraction, ProductRating, Favorite
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'description', 'price', 'category', 'image_url', 'discounted_price', 'has_discount', 'is_pre_order']
    ...
    def get_has_discount(self, obj):
        if not obj.store:
            return False
        return obj.store.discount_percentage > 0
        
    def get_discounted_price(self, obj):
        if not obj.store or obj.store.discount_percentage <= 0:
            return obj.price
        
        from decimal import Decimal
        discount = obj.price * (Decimal(obj.store.discount_percentage) / Decimal('100.0'))
...
class StoreSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Store
        fields = ['id', 'name', 'description', 'categories', 'section', 'products', 'logo', 'logo_url', 'discount_percentage']
...
class ProductCreateSerializer(serializers.ModelSerializer):
    ...
    is_pre_order = serializers.BooleanField(required=False, default=False)
    ...
    def validate_is_pre_order(self, value):
    ...
    def create(self, validated_data):
        ...
        # Ensure is_pre_order is properly converted to boolean
        if 'is_pre_order' in validated_data:
            validated_data['is_pre_order'] = self.validate_is_pre_order(validated_data['is_pre_order'])
...
class StorePaginatedSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Store
        fields = ['id', 'name', 'description', 'categories', 'section', 'logo_url', 'diamonds']
