...
from MallAPI.models.store_model import Store, Category, Product,Section, ProductComment, ProductInteraction, CommentInteraction, ProductRating, Favorite
...
class StoreDiscountView(APIView):
    ...
    def get(self, request):
        """Get current store discount information"""
        try:
            # Get the store owned by the current user
            ...
            return Response({
                ...
                "discount_percentage": store.discount_percentage,
            }, status=status.HTTP_200_OK)
        except Store.DoesNotExist:
            return Response(format_error_message("Store not found"), status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
    ...
    def post(self, request):
        """Set store-wide discount percentage"""
        try:
            # Get the store owned by the current user
            store = get_object_or_404(Store, owner=request.user)
            ...
            try:
                ...
                if discount_percentage < 0 or discount_percentage > 100:
                    return Response(
                        format_error_message("Discount percentage must be between 0 and 100"),
                        status=status.HTTP_400_BAD_REQUEST
                    )
            except (InvalidOperation, TypeError):
                return Response(
                    format_error_message("Invalid discount percentage value"),
                    status=status.HTTP_400_BAD_REQUEST
                )
            
            # Check if this is a new discount or an update
            is_new_discount = store.discount_percentage == 0 and discount_percentage > 0
            
            # Update store discount
            store.discount_percentage = discount_percentage
            store.save()
            
            # Get count of products affected
            product_count = Product.objects.filter(store=store).count()
            
            # Send email notification to customers if this is a new discount
            if is_new_discount:
                from MallAPI.services.email_service import EmailService
                try:
                    success, message = EmailService.send_store_discount_notification(
                        store.name, 
                        discount_percentage
                    )
                    if success:
                        logger.info(f"Email notification sent for store discount: {store.name} {discount_percentage}%")
                    else:
                        logger.warning(f"Failed to send store discount notification: {message}")
                except Exception as e:
                    logger.error(f"Error sending store discount notification: {str(e)}")
            
            return Response({
                "message": f"Discount of {discount_percentage}% applied to {product_count} products",
                "store_id": store.id,
                "store_name": store.name,
                "discount_percentage": store.discount_percentage,
    ...
    def delete(self, request):
        ...
        try:
            ...
            return Response({
                "message": f"Discount removed from {product_count} products",
                "store_id": store.id,
                "store_name": store.name,
                "product_count": product_count
            }, status=status.HTTP_200_OK)
